...
```json
{
  ...
  "tarball_url": "https://raw.githubusercontent.com/leodarshantech/lyncost/main/dist-packages/lyncost-0.1.1-linux-x86_64.tar.gz"
}
```

### 3.2 Rust Backend Architecture (`src-tauri/src/commands.rs`)
1. **`check_app_update()`**:
   - Queries `https://raw.githubusercontent.com/leodarshantech/lyncost/main/version.json` using `curl` with a 4s connection timeout.
...
4. **`version.json`**:
   ```json
   {
     ...
     "tarball_url": "https://raw.githubusercontent.com/leodarshantech/lyncost/main/dist-packages/lyncost-0.1.2-linux-x86_64.tar.gz"
   }
   ```
