...
export const safeGetStorage = (key: string, fallback: string | null = null): string | null => {
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
      return localStorage.getItem(key) ?? fallback;
    }
  } catch (e) {
    console.warn(`[Lyncost] localStorage read denied for key: ${key}`, e);
  }
  return fallback;
};

export const safeSetStorage = (key: string, val: string): void => {
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
      localStorage.setItem(key, val);
    }
  } catch (e) {
    console.warn(`[Lyncost] localStorage write denied for key: ${key}`, e);
  }
};
...
export const useAppStore = create<AppStoreState>((set, get) => ({
  ...
  isPrivacyMode: safeGetStorage('lyncost_privacy_mode') === 'true',
  togglePrivacyMode: () => {
    set((state) => {
      const next = !state.isPrivacyMode;
      safeSetStorage('lyncost_privacy_mode', String(next));
  ...
  initApp: async () => {
    ...
    try {
      ...
      if (settings?.base_currency) {
        ...
        if (!safeGetStorage('lyncost_number_format')) {
          autoSyncNumberFormatWithCurrency(settings.base_currency);
        }
      }
      const savedTheme = safeGetStorage('lyncost_desktop_theme') as 'dark' | 'light' | null;
  ...
  setBaseCurrency: async (newCurrency: string) => {
    try {
      ...
      const updatedSettings = await invoke<AppSettings>('get_app_settings');
      safeSetStorage('lyncost_base_currency', curr);
      if (typeof window !== 'undefined') {
  ...
  setTheme: async (theme: 'dark' | 'light') => {
    set({ theme });
    safeSetStorage('lyncost_desktop_theme', theme);
    if (typeof window !== 'undefined') {
      if (theme === 'light') {
        document.documentElement.classList.add('theme-light');
        document.documentElement.classList.remove('theme-dark');
