...
GITHUB_REPO="leodarshantech/lyncost"
...
if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/src-tauri/target/release/lyncost" ]; then
    ...
    LATEST_VER=$(curl -fsSL --connect-timeout 4 "https://raw.githubusercontent.com/$GITHUB_REPO/main/version.json" 2>/dev/null | grep -o '"version": *"[^"]*"' | head -n 1 | cut -d'"' -f4 || echo "0.1.5")
    [ -z "$LATEST_VER" ] && LATEST_VER="0.1.5"

    RAW_URL="https://raw.githubusercontent.com/$GITHUB_REPO/main/dist-packages/lyncost-${LATEST_VER}-linux-x86_64.tar.gz"
    FALLBACK_RAW_URL="https://raw.githubusercontent.com/$GITHUB_REPO/main/dist-packages/lyncost-0.1.5-linux-x86_64.tar.gz"
...
echo -e "   ${PURPLE}• Android APK:${RESET}      ${CYAN}https://raw.githubusercontent.com/leodarshantech/lyncost/main/dist-packages/lyncost-mobile.apk${RESET} ${DIM}(Beta)${RESET}"
echo ""
