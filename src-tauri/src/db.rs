...
pub fn seed_defaults(conn: &Connection) -> Result<(), String> {
    ...
    let pm_count: i64 = conn.query_row("SELECT COUNT(*) FROM payment_methods", [], |r| r.get(0)).unwrap_or(0);
    if pm_count == 0 {
        let seed_pm = [
        ...
        for (name, type_key, icon, color, is_default) in seed_pm {
            let _ = conn.execute(
                "INSERT OR IGNORE INTO payment_methods (name, type_key, icon, color, is_default, is_active) VALUES (?1, ?2, ?3, ?4, ?5, 1);",
                params![name, type_key, icon, color, is_default],
            );
        }
    }
